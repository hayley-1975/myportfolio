  ...
  const serviceList = [
    {
      ...
      deliverables: [
        "Custom conversion components",
        "Sub-second loading times",
        "A/B split-testing architecture",
      ],
    },
    {
      num: "02",
  ...
  return (
    <div className="p-4 sm:p-8 lg:p-10 max-w-5xl mx-auto space-y-8">
      <div>
        <Link
          href="/"
          className="inline-flex items-center gap-1 text-xs font-semibold text-cerulean-600 hover:text-cerulean-800 mb-4"
        >
        ...
        <p className="text-sm text-surface-muted mt-1 max-w-2xl">
          Everything you need to automate workflows, safeguard data, and scale
      ...
      <div className="space-y-4">
        {serviceList.map((s) => (
          <div
            key={s.num}
            className="bg-white/95 backdrop-blur-md rounded-2xl border border-cerulean-100 shadow-card p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:border-cerulean-300 transition-all"
          >
              ...
              <div className="space-y-1">
                ...
                <div className="pt-2 flex flex-wrap gap-3">
                  {s.deliverables.map((d) => (
                    <span
                      key={d}
                      className="inline-flex items-center gap-1.5 text-[11px] font-medium text-cerulean-800"
                    >
                      <CheckCircle
                        weight="fill"
